# DaxwellOps Analytics — Reliable Data Pipeline + KPI Dashboard (Airflow • dbt • Postgres • Metabase)

A solo end-to-end analytics project that simulates a manufacturing/wholesale business workflow (like Daxwell).  
...
**Demo video (5–10 min):** _<add link>_
...
- `analytics.fact_fulfillment`  
  OTIF flag, shipped vs ordered quantities, order-to-ship / ship-to-deliver lead times
- `analytics.fact_contract_violations`  
  Contract checks (price mismatch, MOQ violations) + estimated revenue impact
...
Indexes for scale
Indexes added on common filter/join columns (example: order_date, customer_id, issue_type).
